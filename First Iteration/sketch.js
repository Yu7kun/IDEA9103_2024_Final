...
function keyPressed() {
  if (key === 'R') {
    // Press 'R' to create a new non-overlapping circle
    let circle = createNonOverlappingCircle();
    if (circle != null) {
      circles.push(circle);
    }
  } else if (key === '1') {
    // Press '1' to set ripple color to red
    rippleColor = color(255, 0, 0); // Red
  } else if (key === '2') {
    // Press '2' to set ripple color to green
    rippleColor = color(0, 255, 0); // Green
  } else if (key === '3') {
    // Press '3' to set ripple color to blue
...
function mousePressed() {
  for (let i = 0; i < circles.length; i++) {
    ...
    if (d < circles[i].diameter / 2) {
      // Create multiple ripples when clicking inside a circle
...
function mouseDragged() {
  // Create a ripple at the mouse position with random color while dragging
